...
class reqinfo:
	...
	def add_bfile(self, bfile):
		self.added.append(bfile)

	def remove_bfile(self, bfile):
		self.removed.append(bfile)
...
def run(filepath):
	...
	while True:
		...
		if l.find("new temp file") != -1:
			...
			rinfo = reqinfos.get(reqid)
			if not rinfo:
				rinfo = reqinfo(reqid)
				reqinfos[reqid] = rinfo
			rinfo.add_bfile(bfile)

		if l.find("removing temp file") != -1:	
			ll = l.split(" ")
			bfile = ll[7].strip()
			...
			if not rinfo:
				rinfo = reqinfo(reqid)
				reqinfos[reqid] = rinfo
			rinfo.remove_bfile(bfile)

		if l.find("remove temp file") != -1 and l.find("unexpected failure") != -1:
			ll = l.split(" ")
			bfile = ll[6].strip()
			binfo = bfiles.get(bfile)
			if not binfo:
	...
	print "Multiple Reqids:", str(multipleReqids.keys())
